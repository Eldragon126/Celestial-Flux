...
var _ability_restore_token: int = 0
...
func _build_body() -> void:
	var hull := get_node_or_null("SeraphHull") as Polygon2D
	if hull == null:
		hull = Polygon2D.new()
	...
	_body_core = get_node_or_null("SeraphCore") as Polygon2D
	if _body_core == null:
	...
	for child in get_children():
		if child is Polygon2D and String(child.name).begins_with("GravityLane"):
			_lane_visuals.append(child as Polygon2D)

	for i in range(_lane_visuals.size(), 4):
...
func _disable_player_abilities() -> void:
	...
	_ability_restore_token += 1
	_restore_player_abilities_later(_ability_restore_token)

func _restore_player_abilities_later(token: int) -> void:
	await get_tree().create_timer(ability_lock_duration).timeout
	if token != _ability_restore_token:
		return
