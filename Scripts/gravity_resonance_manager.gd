...
@export var resonance_visual_alpha_scale: float = 0.82
@export var visual_radius_cap: float = 460.0
...
@export_range(0.0, 1.0, 0.01) var visual_glyph_alpha_cap: float = 0.36
@export var maximum_manual_resonance_zones: int = 3
@export var manual_zone_merge_distance: float = 120.0
@export var visual_min_intensity: float = 0.18
...
func _update_zone_visual(zone: Dictionary, delta: float) -> void:
	...
	var life_alpha := lerpf(0.14, 0.62, intensity) * clampf(resonance_visual_alpha_scale, 0.1, 1.0)
	...
	if core != null:
		...
		core.color = Color(base_color.r, base_color.g, base_color.b, _visual_alpha(life_alpha * 0.13, visual_fill_alpha_cap))
	if ring != null:
		ring.points = _circle_points(_visual_segments(visual_ring_segments), visual_radius)
		ring.width = lerpf(1.6, 3.4, intensity)
	...
	if accent != null:
		...
		accent.width = lerpf(0.9, 1.8, intensity)
		accent.default_color = Color(1.0, 1.0, 1.0, _visual_alpha(life_alpha * 0.28, visual_glyph_alpha_cap))
	if label != null:
