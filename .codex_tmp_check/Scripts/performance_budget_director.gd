...
func _apply_gravity_scar_budget(scene: Node, low: bool, medium: bool) -> void:
	...
	if scars == null:
		return
	_set_if_present(scars, "max_active_scars", 5 if low else (7 if medium else 8))
	_set_if_present(scars, "visual_quality", 1 if low else 2)
	_set_if_present(scars, "max_particles_per_scar", 10 if low else (22 if medium else 36))
	_set_if_present(scars, "max_body_targets_per_tick", 22 if low else (32 if medium else 42))
	_set_if_present(scars, "max_projectile_targets_per_tick", 26 if low else (40 if medium else 54))
	_set_if_present(scars, "field_tick_interval", 0.065 if low else 0.045)
...
func _apply_event_horizon_budget(scene: Node, low: bool) -> void:
	...
	if horizon == null:
		return
	_set_if_present(horizon, "max_targets_per_tick", 36 if low else 72)
	_set_if_present(horizon, "field_tick_interval", 0.04 if low else 0.025)
